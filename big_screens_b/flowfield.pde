class FlowField {
  ...
  ArrayList<PVector> boundOutPointsLeft;
  color debugClr;
  int debugOffset;
  ...
  FlowField(int r) {
    ...
    boundOutPointsLeft = new ArrayList<PVector>();
  ...
  ArrayList<PVector> getBoundOutPointsLeftCtr() {
    return boundOutPointsLeft;
  }
  ...
  void initOutPointsHorz() {
    ...
    println("here");
    initBoundedPointsLeft(PULSECTR);
  }

  void initBoundedPointsLeft(PVector startPos) {
    println("calling init bounds");
    
    float xBound = 0.05 * width; 
    float xUpperBound = startPos.x + xBound;
    float xLowerBound = startPos.x - xBound;
    
    fill(255);
    ellipse(xLowerBound, height/2, 50,50);
    ellipse(xUpperBound, height/2, 50,50);
    println("bounds", xLowerBound, xUpperBound);
    
    int noOfPoints = outPointsLeft.size();

    for (int i = 0; i < noOfPoints; i++) {
      if (outPointsLeft.get(i).x < xUpperBound && outPointsLeft.get(i).x > xLowerBound) {
      fill(255,0,0);
      println(i, outPointsLeft.get(i).x, outPointsLeft.get(i).y);
      ellipse(outPointsLeft.get(i).x, outPointsLeft.get(i).y, 50, 50);
      float newX = outPointsLeft.get(i).x;
      float newY = outPointsLeft.get(i).y;
      PVector newPoint = new PVector(newX, newY);
      boundOutPointsLeft.add(newPoint);
      }
      
    }
    println("boundpoints", boundOutPointsLeft.size());
    println("allpoints", outPointsLeft.size());
