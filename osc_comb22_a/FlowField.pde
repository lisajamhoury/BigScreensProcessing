  ...
  void getOutPoints() {
   for (int x = 0; x < cols; x++) {
      for (int y = 0; y < rows; y++) {
        ...
        if ((x < pulseCtr.x && y > 0.75 * height) && (heading > 0.2 &&heading < 1.4)) {
          println("left", field[x][y]);
          fieldOutPoints.add(field[x][y]);      
        }
        if ((x > pulseCtr.x && y > 0.75 * height)&& (heading > 1.7 && heading < 2.8)) {
          fieldOutPoints.add(field[x][y]);
        }
    }
